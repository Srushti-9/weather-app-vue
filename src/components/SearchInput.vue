  ...
  searchTerm.timeout = setTimeout(async () => {
    if (searchTerm.query !== '') {
      const res = await fetch(
        `http://api.weatherapi.com/v1/search.json?key=69d016292a4045e891d143254241003&q=${searchTerm.query}&aqi=no`
      )
...
const getWeather = async (id) => {
  const res = await fetch(
    `http://api.weatherapi.com/v1/forecast.json?key=69d016292a4045e891d143254241003&q=id:${id}&days=3&aqi=no&alerts=no`
  )
